...
import { getProviderMetadata, canonicalizeRef, fallbackEnvNames } from '../providers/provider-metadata.js'
 ...
 * still accepts them.
 *
 * v2.3.0: per commit 4, this consults the canonical ProviderMetadata
 * table so legacy alias refs (`FIRECRAWL_KEY`, `PARALLELMCP_API_KEY`,
 * …) are still accepted on read-time migration paths.
 *
...
export function providerCredentialRef(providerId, slot) {
  ...
  if (!meta) return ''
  const base = (meta.credential.mode === 'host') ? meta.credential.canonicalRef : `${providerIdToEnvName(providerId)}`
  if (!base) return ''
...
export function providerIdToEnvName(providerId) {
  ...
  // Most providers map PROVIDER → <PROVIDER>_API_KEY. Camel-case ids
  // like `parallelMcp` are upper-cased preserving underscores.
  const upper = providerId.replace(/([a-z])([A-Z])/g, '$1_$2').toUpperCase()
...
export async function resolveCredentialPool(providerId, ctx, maxKeys = 3) {
  ...
  for (let i = 1; i <= slots; i++) {
    ...
    if (credentials && typeof credentials.resolve === 'function') {
      // v2.3.0 ALIAS MIGRATION: try canonical first; on miss, walk
      // every alias ref declared in provider-metadata. The keyed slot
      // stays canonical so downstream pool rotation / cooldown
      // fingerprints see only canonical refs.
      const meta = getProviderMetadata(providerId)
      const aliases = (meta && meta.credential && Array.isArray(meta.credential.aliases))
        ? meta.credential.aliases
        : []
      const tried = await tryResolve(credentials, ref)
      if (tried) {
        resolved = { key: tried.value, source: tried.source || 'credentials', raw: ref }
      } else if (aliases.length > 0) {
        // Slot 1 also tries the bare alias; slots ≥ 2 try the suffixed
        // alias (operators may rotate old spellings).
        const aliasCandidates = i <= 1
          ? aliases.slice()
          : aliases.flatMap((a) => [`${a}_${i}`])
        for (const aliasRef of aliasCandidates) {
          const t = await tryResolve(credentials, aliasRef)
          if (t) {
            resolved = { key: t.value, source: t.source || 'credentials', raw: aliasRef }
            break
          }
        }
      }
...
/**
 * Resolve a single ref through the credentials seam; tolerate any
 * return shape (object or string).
 */
async function tryResolve(credentials, ref) {
  try {
    ...
    if (typeof r === 'string' && r.length > 0) return { value: r }
  } catch { /* fall through */ }
  return null
}

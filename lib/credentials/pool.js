 ...
 */
export function buildPool(providerId, loaded, maxKeys = 3) {
  ...
  for (let i = 1; i <= slots; i++) {
    ...
    if (!resolved || !resolved.key) {
      // Missing credentialRef is its own outcome in §II.3.3 ("Missing
      // credentialRef → try next credentialRef"). We track that as an
      // `unknown` entry that the runner knows to skip without changing
      // state.
    ...
    for (let attempt = 0; attempt <= KEY_RETRY_ONCE; attempt++) {
      try {
        const value = await withTimeout(
          args.fetch(entry.credentialRef, args.signal, entry.credentialRef),
          args.perKeyTimeoutMs,
          args.signal,
        )
        ...
        lastErr = withClass(Object.assign(new Error(safeMsg), { name: (err && err.name) || 'Error', cause: err }), effective)

        if (effective === 'aborted') {
          // Abort propagates up; do not touch state, do not retry.
          throw lastErr
        }
        if (effective === 'security' || effective === 'config' || effective === 'invalid-request') {
          // Hard throw — do not retry this key, do not move to next provider.
          throw lastErr
        }
        if (effective === 'auth') {
        ...
        if (effective === 'credential') {
          // Move on to next credentialRef; do not change state.
          attempts.push({ credentialRef: entry.credentialRef, class: 'credential' })
          lastErr = null
          break
        }
        if (isKeyRetryable(effective) && attempt === 0) {
          // Retry this credential ONCE.
          continue
        }
        // Either non-retryable, or second attempt also failed. Move on.
        if (effective === 'invalid-response') {
          ...
          // Budget overflow exits the whole chain — caller decides.
          args.onPoolUpdated?.(pool)
...
/**
 * Race a promise against a timeout. The timeout rejects with a `transient`
 * error (consistent with SPEC §II.3.3 "5xx / network / timeout").
 *
 * @template T

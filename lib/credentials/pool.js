 ...
 *
 * A slot counts toward `configured` only when it carries a `fingerprint`.
 * `buildPool` writes the fingerprint **iff** it actually resolved a secret
 * into that slot; a slot without one is a placeholder it pushed so that
 * `runPool` still has a full slot list to walk. Reporting placeholders as
 * "configured" is how a run against keyless providers used to turn the
 * doctor panel into fiction: the failed run persisted its placeholder pool
 * via `setPoolState`, and the panel then read
 * "3 configured / 0 healthy / 0 cooldown / 0 invalid" for providers that
 * have no credential at all.
 *
...
export function poolSummary(pool) {
  ...
  for (const e of pool) {
    ...
    // No fingerprint → the slot was never filled → not a configured
    // credential, and not a measurement of anything either.

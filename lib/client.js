window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── All providers this plugin can write keys for. Sourced from
    //    lib/providers/provider-metadata.js; the BEGIN/END markers
    //    below delimit a generated block. lib/credentials/resolve.js
    //    walks the alias list declared in metadata when an operator
    //    used a legacy spelling (FIRECRAWL_KEY, PARALLELMCP_API_KEY).
    //    To regenerate: `node scripts/sync-provider-client.mjs`.
    // ──
...
var PROVIDERS = [
      ...
      { id: "firecrawl", env: "FIRECRAWL_API_KEY" },
      ...
      { id: "parallelMcp", env: "PARALLEL_MCP_API_KEY" },
    ];
    // END GENERATED TRINITY PROVIDERS
    // ── duckduckgo intentionally absent: it has no API key.
    // ── openai intentionally absent: Hosted Search half-implementation
    //    was removed in v2.3.0 (spec § 六 / review round 2).
    // ── alias spellings (FIRECRAWL_KEY, PARALLELMCP_API_KEY) replaced
    //    by their canonical names. lib/credentials/resolve.js still
    //    honours the legacy spellings at pool-resolve time.

...
export async function fetch(request, signal, ctx) {
  ...
  if (ctx && ctx.body instanceof Uint8Array) {
    ...
    if (safe.statusCode >= 400) {
      throw toolError('rss', 'HTTP_' + safe.statusCode, `rss feed returned HTTP ${safe.statusCode}`, 'verify the URL is a valid feed')
    }
    text = new TextDecoder('utf-8').decode(safe.bytes)
  }

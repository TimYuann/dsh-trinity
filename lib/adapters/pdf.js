...
export async function fetch(request, signal, ctx) {
  ...
  if (ctx && ctx.body instanceof Uint8Array) {
    ...
    if (safe.statusCode >= 400) {
      throw toolError('pdf', 'HTTP_' + safe.statusCode, `pdf url returned HTTP ${safe.statusCode}`, 'verify the URL serves a PDF')
    }
    contentType = safe.contentType || ''
    statusCode = safe.statusCode
    finalUrlStr = (safe.finalUrl && typeof safe.finalUrl.toString === 'function') ? safe.finalUrl.toString() : url
    bytes = safe.bytes
  }

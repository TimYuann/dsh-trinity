...
/**
 * Words that mark a sentence as the model narrating rather than searching.
 *
 * These are matched ANYWHERE in the line, not just at its start: the first
 * live run of the fix (headless, 2026-09-12) dropped the leading prose but
 * still emitted
 *   "distributed shell? Profile bundle patch? Need infer. Could be about"
 * as a sub-query — a reasoning fragment that begins with a plausible
 * noun phrase. A query does not speculate about itself, so an explicit
 * word-boundary match on the verbs of speculation is a safe filter;
 * `\binfer\b` still lets "inference engine" through.
 */
const NARRATION_PATTERNS = [
  /\bneed(s|ed)? (to )?infer\b/,
  /\bcould (be|mean) about\b/,
  /\bnot sure\b/,
  /\bi think\b/,
  /\bwe think\b/,
  /\blet me\b/,
...
/** A query that stops on a function word was cut off mid-sentence. */
const DANGLING_TAIL = /\b(about|the|of|to|is|are|was|were|be|been|in|on|at|for|with|that|which|and|or|a|an|by|as|from|into|it|this|these|those|could|would|should|can|will|need|needs)$/i
...
function isQueryShaped(s, lenient) {
  ...
  if (NARRATION_PATTERNS.some((re) => re.test(t))) return false
  if (DANGLING_TAIL.test(t)) return false

...
/**
 * Tokenize text into a stream of overlapping tokens. v2.3.0 § Commit 5:
 *   - lowercase ASCII word tokens (length 3..30)
 *   - Unicode letter/number tokens (any script, including CJK wide
 *     runs and accented Latin)
 *   - CJK character bigrams as a fallback for languages without
 *     whitespace
 *
 * The bigram path is intentionally cheap; it does not bring in a
 * segmenter dependency. Tests assert at least one CJK token is emitted
 * for plain Chinese input.
 *
 * @param {string} text
 * @returns {string[]}
 */
export function tokenize(text) {
  ...
  for (const w of asciiWords) {
    if (w.length > 2 && w.length < 30) out.push(w)
  }
  const unicodeWords = text.match(/[\p{L}\p{N}]+/gu) || []
  for (const w of unicodeWords) {
    if (w.length > 1 && w.length < 30) out.push(w)
  }
...
function breakIntoPassages(body) {
  // Two passes: split on sentence boundaries (ASCII . ! ? plus CJK 。！？),
  // then merge short sentences into 200-char windows per SPEC §II.5.
  const out = []
  let cursor = 0

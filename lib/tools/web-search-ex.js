...
/** Max `## Provider errors` lines rendered before collapsing to a count. */
const PROVIDER_ERROR_RENDER_CAP = 12
...
export const OUTPUT = {
  ...
  render(_args, value) {
    const blocks = []
    // Defect C: which provider actually served this call was in the
    // structured value all along (schema `provider`), but never rendered —
    // so `auto` and ordered-list routing were invisible in the tool output
    // and could only be inferred by re-running with a pinned provider.
    if (value && typeof value.provider === 'string' && value.provider.length > 0) {
      // Trailing newline: the host concatenates render blocks verbatim, so
      // without it the line reads "provider: exa[first source](…)" — the id
      // glued to the first URL (observed in the headless run, 2026-09-12).
      blocks.push({ type: 'text', text: `provider: ${value.provider}\n` })
    }
    ...
    if (value && Array.isArray(value.providerErrors) && value.providerErrors.length > 0) {
      // The structured list stays complete; only the rendering is capped,
      // so a bad credential set can never paste dozens of identical
      // `key:credential` lines into the conversation. With the aggregate
      // fan-out now filtered to credentialed providers, the lone
      // "15 keyless providers × 3 slots" wall can no longer be produced.
      const shown = value.providerErrors.slice(0, PROVIDER_ERROR_RENDER_CAP)
      const hidden = value.providerErrors.length - shown.length
      blocks.push({
        type: 'text',
        text: '\n## Provider errors\n' + shown.map((e) => `- ${e.provider}: ${e.error}`).join('\n') +
          (hidden > 0 ? `\n- … and ${hidden} more (see value.providerErrors)` : ''),
      })
    }
    return blocks
  },

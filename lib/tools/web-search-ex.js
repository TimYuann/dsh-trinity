...
export function createTool(opts) {
  return defineTool({
    ...
    async execute(args, exec) {
      ...
      // P0 #8: routing is a union — string ('auto' / 'aggregate' / provider
      // id) OR array of provider ids. Schema declares type:'json' so DSH
      // passes any value through; we validate here.
      const routing = normaliseRouting(args.routing)
...
function normaliseRouting(routing) {
  ...
  throw toolError('web_search_ex', 'INVALID_INPUT',
    `routing must be a string or array of strings (got ${typeof routing})`,
    "pass routing='auto' | 'aggregate' | '<providerId>' | ['brave','exa',...]")
}

// lib/providers/search/search1api.js — POST https://api.search1api.com/search
...
export const PROVIDER_ID = 'search1api'
...
export async function providerSearch(query, numResults, apiKey, signal) {
  if (!apiKey) {
    throw toolError(
      ...
      'MISSING_API_KEY',
      'missing API key',
      "set SEARCH1API_API_KEY via /webdoctor-keys set search1api <key> (credential ref: SEARCH1API_API_KEY), or pass $SEARCH1API_API_KEY env var, or use a different provider (anysearch / gemini / brave) via web_search_ex(provider='...')",
    )
  }

// lib/providers/search/ollama.js — POST https://ollama.com/api/web_search
...
export async function providerSearch(query, numResults, apiKey, signal) {
  if (!apiKey) {
    throw toolError(
      ...
      'MISSING_API_KEY',
      'missing API key',
      "set OLLAMA_API_KEY via /webdoctor-keys set ollama <key> (credential ref: OLLAMA_API_KEY), or pass $OLLAMA_API_KEY env var, or use a different provider (anysearch / gemini / brave) via web_search_ex(provider='...')",
    )
  }

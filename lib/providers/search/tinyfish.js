// lib/providers/search/tinyfish.js — POST https://api.tinyfish.io/v1/search
...
export const PROVIDER_ID = 'tinyfish'
...
export async function providerSearch(query, numResults, apiKey, signal) {
  if (!apiKey) {
    throw toolError(
      ...
      'MISSING_API_KEY',
      'missing API key',
      "set TINYFISH_API_KEY via /webdoctor-keys set tinyfish <key> (credential ref: TINYFISH_API_KEY), or pass $TINYFISH_API_KEY env var, or use a different provider (anysearch / gemini / brave) via web_search_ex(provider='...')",
    )
  }

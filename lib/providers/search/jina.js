// lib/providers/search/jina.js — GET https://s.jina.ai/?q=...
...
export async function providerSearch(query, numResults, apiKey, signal) {
  if (!apiKey) {
    throw toolError(
      ...
      'MISSING_API_KEY',
      'missing API key',
      "set JINA_API_KEY via /webdoctor-keys set jina <key> (credential ref: JINA_API_KEY), or pass $JINA_API_KEY env var, or use a different provider (anysearch / gemini / brave) via web_search_ex(provider='...')",
    )
  }
  const url = `https://s.jina.ai/?q=${encodeURIComponent(query)}&num=${Math.min(numResults, 20)}`

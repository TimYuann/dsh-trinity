// lib/providers/search/perplexity.js — POST https://api.perplexity.ai/search
...
export const PROVIDER_ID = 'perplexity'
...
export async function providerSearch(query, numResults, apiKey, signal) {
  if (!apiKey) {
    throw toolError(
      ...
      'MISSING_API_KEY',
      'missing API key',
      "set PERPLEXITY_API_KEY via /webdoctor-keys set perplexity <key> (credential ref: PERPLEXITY_API_KEY), or pass $PERPLEXITY_API_KEY env var, or use a different provider (anysearch / gemini / brave) via web_search_ex(provider='...')",
    )
  }

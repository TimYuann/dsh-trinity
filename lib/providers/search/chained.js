...
import { selectRouting as parseRouting } from './select-routing.js'
...
export const PROVIDER_REGISTRY = {
  searxng, exa, brave, parallel, tinyfish, search1api, searchinfinity,
...
export const PROVIDER_CREDENTIAL_MAP = {
  ...
  firecrawl: { fieldName: 'firecrawlApiKey', envName: 'FIRECRAWL_API_KEY' },
 ...
 *
 * @deprecated v2.3.0 — kept only so v1 test fixtures still compile. The
 * production path reads `settings.routing` directly through the live
 * `SettingsScope`; this helper is no longer consulted per call.
 *
 * @param {any} cfg
...
/**
 * Re-export the canonical routing parser. Existing imports of
 * `selectRouting` from this module continue to work; the actual
 * implementation lives in `./select-routing.js` and is the one and
 * only router.
 *
...
export function selectRouting(routing, opts) {
  // Use the caller's opts when provided (unit-test seam), default to
  // the canonical provider-id set in production paths.
  const knownIds = (opts && Array.isArray(opts.knownIds))
    ? opts.knownIds
    : ALL_PROVIDER_IDS
  return parseRouting(routing, { knownIds })
}
...
export async function chainedSearch(request, signal, ctx) {
  ...
  // v2.3.0: routing precedence is request.routing (raw, unparsed) → live
  // settings.routing. v1 `searchProviderOrder` is no longer consulted on
  // every call.
  const cfg = (ctx && ctx.config && typeof ctx.config === 'object') ? ctx.config : {}
  const explicitRouting = (request && request.routing !== undefined) ? request.routing : (cfg.routing || 'auto')
  const routing = selectRouting(explicitRouting)
...
async function runAggregate(query, maxResults, signal, ctx, keys, budgets, deadline) {
  ...
  // v2.3.0 § Commit 3.7: a fatal error (security / config / aborted /
  // invalid-request) aborts sibling workers and is rethrown unchanged.
  // Fallbackable errors are collected and aggregate proceeds.
  //
  // 2026-09-12 (defect A): "eligible" means the provider can actually be
  // attempted — the auto chain is filtered by credential availability
  // first. Fanning out to keyless providers spent the maxProviders budget
  // on providers that cannot succeed, printed one `credential` line per
  // slot, and wrote placeholder pools that the doctor later read back as
  // "3 configured".
  const autoChain = buildAutoChainList(ctx)
  const providerList = await eligibleForFanOut(autoChain, ctx, budgets)
  if (providerList.length === 0) {
    const e = new Error(
      `aggregate: none of the ${autoChain.length} eligible providers has credentials configured ` +
      `(${autoChain.join(', ')})`,
    )
    e.code = 'WEB_PROVIDER_ERROR'
    e.attempts = []
    e.checkedProviders = autoChain
    e.doctorRecommended = true
    throw e
  }
  ...
  /** @type {Map<string, any[]>} */
  const pools = new Map()
  /** @type {{ err: Error, worker: Promise<any> } | null} */
  let fatalError = null
  ...
  const worker = async () => {
    while (queue.length > 0) {
      ...
      // The budget counts providers actually attempted. The queue is
      // pre-filtered to credentialed providers, so a keyless row can no
      // longer consume a slot here.
      ...
      let r
      try {
        r = await tryProvider(providerId, query, maxResults, signal, ctx, keys, budgets, deadline)
      } catch (e) {
        const cls = classifyError(e)
        if (cls === 'security' || cls === 'config' || cls === 'aborted' || cls === 'invalid-request') {
          if (!fatalError) fatalError = { err: e }
          try { signal && typeof signal.abort === 'function' && signal.abort() } catch { /* ignore */ }
          throw e
        }
        // Fallbackable error — record and continue.
        providerErrors.push({ provider: providerId, error: e && e.message ? e.message : String(e) })
        continue
      }
      if (r.ok) {
  ...
  // Drain all workers; if any threw, awaitAll is fine — Promise.allSettled
  // lets siblings settle while we capture the first fatal in fatalError.
  const results = await Promise.allSettled(workers)
  for (const r of results) {
    if (r.status === 'rejected') {
      if (!fatalError) fatalError = { err: r.reason }
    }
  }
  for (const [id, p] of pools) setPoolState(id, p)

  if (fatalError) throw fatalError.err
...
async function tryProvider(providerId, query, maxResults, signal, ctx, keys, budgets, deadline) {
  ...
  if (credentialMode === 'pool') {
    ...
    pool = buildPool(providerId, { resolved }, budgets.maxKeys, getPoolState(providerId))
...
/**
 * Can this provider serve a request as configured right now?
 *
 * `buildAutoChainList` answers "which providers COULD serve a search",
 * not "which ones can serve it right now" — it returns every
 * auto-eligible provider whether or not a credential exists for it. The
 * aggregate fan-out must not treat those two questions as the same one:
 * a keyless provider cannot succeed, yet attempting it
 *   - spends the `maxProvidersPerSearch` budget,
 *   - emits one `credential` failure per slot (the ~45-line
 *     "Provider errors" wall for a 15-keyless-provider chain), and
 *   - persists its placeholder pool via `setPoolState`, which is what
 *     made `web_doctor` report "3 configured" for providers with no key.
 *
 * Eligibility follows the same resolution paths `resolveProviderKey`
 * uses, so a provider configured any supported way stays in the list:
 *   1. credential-mode 'none'  → needs nothing
 *   2. credential-mode 'host'  → SearXNG host configured
 *   3. pool slots that resolved a key
 *   4. the `rawConfig.<fieldName>` / bare `process.env.<ENV_NAME>`
 *      last-resort fallbacks
 *
 * @param {string} providerId
 * @param {any} ctx
 * @param {{ maxKeys: number }} budgets
 * @returns {Promise<boolean>}
 */
async function providerCanBeAttempted(providerId, ctx, budgets) {
  const meta = getProviderMetadata(providerId)
  const mode = (meta && meta.credential && meta.credential.mode) || 'api-key'
  if (mode === 'none') return true
  if (mode === 'host') return !!(await resolveSearxngHost(ctx))

  const resolved = await resolveProviderCredentials(providerId, ctx, budgets.maxKeys)
  for (const slot of Object.values(resolved)) {
    if (slot && typeof slot.key === 'string' && slot.key.length > 0) return true
  }
  const map = PROVIDER_CREDENTIAL_MAP[providerId]
  if (map && map.fieldName) {
    const cfgValue = (ctx && ctx.rawConfig) ? ctx.rawConfig[map.fieldName] : undefined
    if (typeof cfgValue === 'string' && cfgValue.length > 0) return true
  }
  if (map && map.envName) {
    const env = process.env[map.envName]
...
/**
 * Narrow a provider list to the ones that can actually be attempted.
 * Resolution is local (no network) and runs in parallel; it is
 * deliberately fail-open — if the credential seam is missing or throws,
 * the provider stays in the list and behaviour matches the pre-fix
 * fan-out rather than silently dropping a working provider.
 *
 * @param {string[]} list
 * @param {any} ctx
 * @param {{ maxKeys: number }} budgets
 ...
 */
async function eligibleForFanOut(list, ctx, budgets) {
  const verdicts = await Promise.all(list.map(async (id) => {
    try {
      return await providerCanBeAttempted(id, ctx, budgets)
    } catch {
      return true
    }
  }))

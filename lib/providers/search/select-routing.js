...
export function selectRouting(routing, opts) {
  // v2.3.0 review § P1#6: the parser MUST refuse unknown ids and
  // MUST refuse provider-id routing when no known-id set is supplied.
  // Empty knownIds is treated as "the chain was not initialised": the
  // safe outcomes are 'auto' and 'aggregate' (no provider pin); any
  // provider-id or array input raises BAD_REQUEST.
  const knownIds = (opts && Array.isArray(opts.knownIds)) ? new Set(opts.knownIds) : null
  const known = (s) => Boolean(knownIds && knownIds.has(s))

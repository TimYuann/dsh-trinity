// lib/providers/search/kagi.js — GET https://kagi.com/api/v0/search?q=...
...
export async function providerSearch(query, numResults, apiKey, signal) {
  if (!apiKey) {
    throw toolError(
      ...
      'MISSING_API_KEY',
      'missing API key',
      "set KAGI_API_KEY via /webdoctor-keys set kagi <key> (credential ref: KAGI_API_KEY), or pass $KAGI_API_KEY env var, or use a different provider (anysearch / gemini / brave) via web_search_ex(provider='...')",
    )
  }
  const url = `https://kagi.com/api/v0/search?q=${encodeURIComponent(query)}&limit=${Math.min(numResults, 20)}`
  ...
  const sources = results
    ...
    .map((r) => {
      ...
      if (r.published) out.publishedAt = toIso8601(r.published) || undefined
      return out
    })
  if (sources.length === 0) {
    throw toolError(

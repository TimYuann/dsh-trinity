...
export function authHeadersFor(profile, value) {
  if (!profile || !value || typeof value !== 'string') return {}
  if (profile.type === 'bearer') return { Authorization: `Bearer ${value}` }
  if (profile.type === 'basic') {
    // Per v2.3.0 § Commit 3.5, Basic credentials MUST be Base64-encoded
    // before being placed in the header (RFC 7617). Plain user:pass is
    // not safe to transmit and was the bug called out in the review
    // P0 #2.4.
